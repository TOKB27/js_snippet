...
export const SystemBPage: React.FC<SystemBPageProps> = ({ onBack }) => {
	...
	const events: SportsEvent[] = [
		{
			id: "EVT-101",
			eventName: "市民フットサル大会（予選リーグ）",
			facilityName: "中央スポーツアリーナ - コートA",
			organizer: "地域スポーツ振興課",
		...
		{
			id: "EVT-102",
			eventName: "ジュニアテニス強化合宿",
			facilityName: "緑ヶ丘テニスコート（クレー）",
			organizer: "テニス協会 事務局",
		...
		{
			id: "EVT-103",
			eventName: "シニアアクアエクササイズ定期教室",
			facilityName: "市民温水プール - 2レーン貸切",
			organizer: "健康増進推進チーム",
		...
		<Container maxWidth="xl" sx={{ py: 4 }}>
			...
			>
				<GenericDataGrid
					rows={events}
					columns={columns}
					columnGroupingModel={sportsColumnGroupingModel}
					loading={isLoading}
				/>
			</Box>
		</Container>
